...
function parser(opts) {
  ...
  */
    ...
    function convertJSXIdentifier(path, node, parent) {
        if (t.isJSXIdentifier(node)) {
            if (node.name === 'this' && t.isReferenced(node, parent)) {
                return t.thisExpression();
            } else if (path.scope.hasBinding(node.name)) {
            ...
            return t.memberExpression(
                convertJSXIdentifier(path, node.object, node),
                convertJSXIdentifier(path, node.property, node)
            );
        }
    ...
    function buildElementCall(path, file) {
        ...
        var tagExpr = convertJSXIdentifier(path, path.node.name, path.node);
        var args = [];
    ...
    function groupAttributes(props, state) {
        var attributes = [];
        var modules = (state.opts.modules || []).reduce(function (modules, name) {
            modules[name] = true;
            return modules;
        }, {});
        ...
        props.forEach(function(prop) {
            ...
            if (propNameSplitOnDash.length > 1) { // contains -
                ...
                if (prefix === 'data' && modules.hasOwnProperty('dataset')) {
                    prefix = 'dataset';
                }
                ...
                if (!propertyWithPrefix) {
                    propertyWithPrefix = t.objectProperty(
                        t.identifier(prefix),
                        t.objectExpression([suffixProperty])
                    );
                    attributes.push(propertyWithPrefix);
                } else {
                    propertyWithPrefix.value.properties.push(suffixProperty);
                }
            } else if (modules.hasOwnProperty(propName) || propName.endsWith('_')) {
                attributes.push(t.objectProperty(t.identifier(propName.replace(/_$/, '')), prop.value));
            } else if (propName === 'value' && modules.hasOwnProperty('props')) {
                // push to props property
                var propsProperty = attributes.find(p => p.key.name === 'props');
                if (!propsProperty) {
                    propsProperty = t.objectProperty(t.identifier('props'), t.objectExpression([prop]));
                    attributes.push(propsProperty);
                } else {
                    propsProperty.value.properties.push(prop);
                }
            } else {
    ...
    function buildOpeningElementAttributes(attribs, file) {
        ...
        if (objs.length === 1) {
            // only one object
            attribs = objs[0];
            attribs = t.objectExpression(groupAttributes(attribs.properties, file));
